...
- One-click learning: add a labeled patch, persist it to [`pixelai_examples.bin`](pixelai_examples.bin), and keep timestamped backups beside the model file according to the `BackupRetention` setting in [`pixelai.ini`](pixelai.ini).
...
## Build
1. Open [`VisualRecognition.sln`](VisualRecognition.sln) in Visual Studio 2026 (MSVC v146 toolset) on Windows.
2. Ensure the MSVC toolchain supports C++23 modules.
3. Build the `VisualRecognition` project; the entry point is [`VisualRecognition/src/main.cpp`](VisualRecognition/src/main.cpp), which imports the `ui` module.

## Run
- Launch the built executable on Windows.
- Use the buttons in the UI or hooks:
  - **Left click** (outside the window): capture a patch around the cursor and store a BMP in [`captures/`](captures/) under the app directory.
...
- **Learn Label**: prompt for a label, persist the model to [`pixelai_examples.bin`](pixelai_examples.bin) in the app directory, and write timestamped backups next to it as governed by `BackupRetention` in [`pixelai.ini`](pixelai.ini).
...
- The loader rejects corrupt or oversized [`pixelai_examples.bin`](pixelai_examples.bin) files: dimensions must fit within reasonable bounds, label text is capped, and the pixel count must match the expected width × height.
...
### Configuration
- Optional app-level settings live in [`pixelai.ini`](pixelai.ini) alongside the executable. Omit the file entirely to keep defaults.
- `BackupRetention` controls how many timestamped copies of [`pixelai_examples.bin`](pixelai_examples.bin) to keep in the app directory. Missing, non-numeric, or non-positive values fall back to the default of 5 backups.

## Project Layout
- [`VisualRecognition/src/ui.ixx`](VisualRecognition/src/ui.ixx): Win32 UI, capture logic, and interaction with the recognizer.
- [`VisualRecognition/src/pixelai.ixx`](VisualRecognition/src/pixelai.ixx): Example-based pixel recognizer and model persistence.
- [`VisualRecognition/src/main.cpp`](VisualRecognition/src/main.cpp): Entry point that launches the UI.
- [`VERSION`](VERSION): Semantic version for releases.
- [`docs/`](docs/): Developer-facing documentation, including checklists and roadmap.

## Changelog
- See [`docs/CHANGELOG.md`](docs/CHANGELOG.md) for release notes.

## Contributing
New to the codebase? Start with [`docs/CONTRIBUTOR_QUICKSTART.md`](docs/CONTRIBUTOR_QUICKSTART.md) for a runtime flow overview. See [`docs/DEVELOPMENT_CHECKLIST.md`](docs/DEVELOPMENT_CHECKLIST.md) for the steps to follow when making changes. Update [`VERSION`](VERSION) and documentation alongside functional updates to keep releases traceable.
